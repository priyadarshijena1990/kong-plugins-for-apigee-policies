...
local GraphQLSecurityFilterHandler = BasePlugin:extend("graphql-security-filter")

function GraphQLSecurityFilterHandler:new()
  return GraphQLSecurityFilterHandler.super.new(self, "graphql-security-filter")
end
...
function GraphQLSecurityFilterHandler:access(conf)
  GraphQLSecurityFilterHandler.super.access(self)
  ...
  if not graphql_query_string or graphql_query_string == "" then
    kong.log.debug("GraphQLSecurityFilter: No GraphQL query string found. Skipping processing.")
  ...
  if #conf.allowed_operation_types > 0 then
    if not operation_type then
      kong.log.warn("GraphQLSecurityFilter: Operation type not detected. Blocking request.")
    ...
    if not is_allowed then
      kong.log.warn("GraphQLSecurityFilter: Operation type '", operation_type, "' is not allowed. Blocking request.")
      return kong.response.exit(conf.block_status, conf.block_body)
    end
  end
  ...
  for _, block_pattern in ipairs(conf.block_patterns) do
    ...
    if ok and res ~= nil then
      kong.log.warn("GraphQLSecurityFilter: Blocking request due to block pattern match: ", block_pattern)
      return kong.response.exit(conf.block_status, conf.block_body)
    elseif not ok then
      kong.log.err("GraphQLSecurityFilter: Error applying block pattern '", block_pattern, "'. Error: ", err)
      -- Decide whether to block on regex error or continue. For security, blocking is safer.
      return kong.response.exit(500, "Internal error in GraphQL security policy.")
    end
  end
  ...
  if conf.extract_operation_type_to_shared_context_key and operation_type then
    ...
    kong.log.debug("GraphQLSecurityFilter: Extracted operation type '", operation_type, "' to shared context key '", conf.extract_operation_type_to_shared_context_key, "'")
  end

  kong.log.debug("GraphQLSecurityFilter: GraphQL request processed. Operation type: ", operation_type or "N/A")
end

return GraphQLSecurityFilterHandler
